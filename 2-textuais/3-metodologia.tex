...
Neste capítulo, serão detalhadas as metodologias empregadas para (a) coletar e organizar as entradas do usuário, (b) construir o modelo do imóvel, (c) executar o pipeline de dimensionamento e (d) validar o atendimento às normas técnicas, culminando na geração do memorial de cálculo.
...
\begin{figure}[h!]
	...
	\UFCfig{}{
	    \includegraphics[width=\textwidth]{figuras/bpmn.png}
	}{
...
O funcionamento do método proposto depende, inicialmente, da aquisição e estruturação das informações do projeto elétrico residencial. Nesta etapa, o objetivo é transformar entradas não estruturadas (texto em linguagem natural e/ou imagem da planta) em um modelo de dados estruturado, contendo os parâmetros mínimos necessários para a execução dos dimensionamentos previstos no escopo do trabalho.
...
No modo de entrada por imagem, o usuário fornece uma planta baixa que contenha, preferencialmente, identificação dos ambientes e dimensões (comprimento e largura). A partir dessa entrada, o sistema realiza a interpretação da planta para extrair dados geométricos relevantes ao dimensionamento, como:
...
Considera-se que o escopo está ``fechado'' quando o modelo do imóvel contém informações suficientes para iniciar o pipeline de dimensionamento, isto é:
...
(ii) um mecanismo de consulta e fundamentação normativa, utilizado para justificar escolhas e responder dúvidas do usuário sobre o porquê de determinadas decisões.
...
\subsection{Organização do pipeline de dimensionamento}

O dimensionamento é estruturado em etapas sequenciais, refletindo o fluxo de projeto e reduzindo dependências circulares. De forma geral, o pipeline é organizado como:

\begin{enumerate}
    \item \textbf{Cálculo das cargas por ambiente:} determinação das cargas mínimas de iluminação e das cargas de tomadas de uso geral (TUG) por ambiente, conforme regras normativas para cada tipo de cômodo.
    \item \textbf{Incorporação de cargas específicas (TUE):} registro de cargas informadas pelo usuário que devem receber tratamento dedicado no projeto.
    \item \textbf{Divisão em circuitos terminais:} organização da instalação em circuitos de iluminação e circuitos de tomadas, com separação funcional e aplicação de critérios normativos e boas práticas para limitação de potência por circuito, quando necessário.
    \item \textbf{Dimensionamento de condutores:} seleção da seção dos condutores por circuito, considerando como critério principal a capacidade de condução de corrente, dadas as premissas adotadas (material do condutor, tipo de isolação, método de instalação e condições de referência).
    \item \textbf{Dimensionamento dos dispositivos de proteção:} escolha dos dispositivos de proteção compatíveis com os circuitos dimensionados, preservando coerência técnica entre proteção e condutores.
    \item \textbf{Balanceamento de fases (quando aplicável):} distribuição dos circuitos entre fases buscando equilíbrio de potência por fase, conforme boas práticas e diretrizes aplicáveis.
    \item \textbf{Verificação de conformidade e ajustes:} checagem do atendimento às regras adotadas, com retorno às etapas anteriores quando necessário.
\end{enumerate}
...
\section{Pipeline de dimensionamento do projeto elétrico}
O dimensionamento do projeto elétrico residencial proposto neste trabalho é conduzido por um pipeline sequencial, fundamentado na previsão de cargas mínimas e na organização em circuitos terminais, conforme critérios normativos e boas práticas usuais. O levantamento das potências é realizado por meio de uma previsão das cargas mínimas de iluminação e tomadas, permitindo determinar a potência total prevista da instalação. Essa previsão segue o que estabelece o item 9.5.2 da ABNT NBR 5410:2008, que orienta o procedimento de estimativa das cargas a serem instaladas.

\subsection{Previsão de carga de iluminação}
A carga mínima de iluminação por cômodo/dependência é estimada em função da área do ambiente, conforme o \textbf{item 9.5.2} da ABNT NBR 5410:2008. Para fins de aplicação metodológica, adota-se:

\begin{itemize}
    \item Para ambientes com \textbf{área $\le$ 6 m$^2$}: prever 100 VA de carga mínima de iluminação.
    \item Para ambientes com \textbf{área $>$ 6 m$^2$}: prever 100 VA para os primeiros 6 m$^2$ \textbf{acrescida de 60 VA a cada aumento de 4 m$^2$ inteiros}.
\end{itemize}
...
\textbf{Observação normativa:} além da potência, a norma estabelece que, para cada cômodo ou dependência, deve ser previsto \textbf{pelo menos um ponto de luz fixo no teto}, comandado por interruptor, conforme o \textbf{subitem 9.5.2.1.1} da ABNT NBR 5410:2008.

\subsection{Precisão de tomadas de uso geral}
A previsão de TUG é realizada conforme o item 9.5.2 da ABNT NBR 5410:2008, diferenciando-se por tipo de ambiente. Além disso, no caso de banheiros e demais situações com restrições, deve-se respeitar as condições de instalação indicadas no item 9.1 (restrições/zonas aplicáveis ao ambiente).m}[]

De forma resumida, a metodologia aplica:

\begin{itemize}
    \item \textbf{Banheiros:} prever pelo menos \textbf{1 tomada} próxima ao lavatório, respeitando as restrições do \textbf{item 9.1}; atribuir \textbf{600 VA por tomada} (mínimo).
    \item \textbf{Cozinhas, copas, copas-cozinhas, áreas de serviço, lavanderias e locais semelhantes:} prever no mínimo \textbf{1 tomada a cada 3,5 m de perímetro} (ou fração); atribuir \textbf{600 VA por ponto para até 3 pontos} e \textbf{100 VA por ponto excedente}, considerando cada ambiente separadamente.
    \item \textbf{Varandas:} prever pelo menos \textbf{1 tomada} (admitindo-se posição próxima ao acesso em situações construtivas específicas); atribuir \textbf{100 VA por tomada}.
    \item \textbf{Salas e dormitórios:} prever no mínimo \textbf{1 tomada a cada 5 m de perímetro} (ou fração); atribuir \textbf{100 VA por tomada}.
    \item \textbf{Demais cômodos/dependências:} se área $\le$ 6 m$^2$, prever ao menos 1 tomada; se área $>$ 6 m$^2$, prever \textbf{1 tomada a cada 5 m de perímetro} (ou fração); atribuir \textbf{100 VA por tomada}.
\end{itemize}
...
\begin{table}[h!]	
	...
	    		Tipo de ambiente & Regra mínima de quantidade & Potência atribuída \\
		...
		\Fonte{Elaborado pelo autor (baseado na NBR 5410).}
    }
\end{table}
...
As cargas específicas (TUE) são incorporadas ao modelo do projeto a partir das informações fornecidas pelo usuário (ex.: condicionador de ar, chuveiro elétrico, máquina de lavar). Conforme o \textbf{subitem 9.5.3.1} da ABNT NBR 5410:2008, todo ponto de utilização previsto para alimentar equipamento de modo exclusivo ou virtualmente dedicado, com corrente nominal superior a 10 A, deve constituir um circuito independente.

Na prática metodológica, isso implica:

\begin{itemize}
    \item \textbf{registrar cada TUE} com sua localização (ambiente) e característica de potência/corrente;
    \item \textbf{alocar cada TUE em circuito terminal próprio}, evitando compartilhamento com TUG;
    \item \textbf{permitir (por critério de projetista)} a criação de circuitos dedicados também para cargas com corrente inferior a 10 A quando houver justificativa técnica (por exemplo, natureza motriz ou sensibilidade eletrônica), mantendo coerência com a boa prática.
\end{itemize}
...
\begin{enumerate}
    \item[\textbf{(a)}] \textbf{Separação funcional:} circuitos de \textbf{iluminação} devem ser separados de circuitos de \textbf{tomadas de uso geral} (boa prática consistente com projetos residenciais).
    
    \item[\textbf{(b)}] \textbf{Circuitos exclusivos para TUE (subitem 9.5.3.1):} TUE alocadas em circuitos independentes quando aplicável (corrente nominal $> 10$ A ou carga dedicada/virtualmente dedicada).
    
    \item[\textbf{(c)}] \textbf{Tomadas de cozinhas e áreas semelhantes (item 9.5.3.2):} os pontos de tomada de cozinhas, copas-cozinhas, áreas de serviço, lavanderias e locais semelhantes devem ser atendidos por circuitos destinados \textbf{exclusivamente} às tomadas desses locais.
    
    \item[\textbf{(d)}] \textbf{Limitação de potência por circuito:} para manter o projeto coerente com faixas usuais de proteção, adota-se a limitação de potência como boa prática:
    \begin{itemize}
        \item \textbf{Iluminação:} limitar a potência por circuito conforme prática didática e valores de referência por tensão.
        \item \textbf{TUG:} limitar a potência por circuito considerando a capacidade dos dispositivos de proteção e condutores.
    \end{itemize}
    ...
    \item[\textbf{(e)}] \textbf{Distribuição entre fases:} em instalações bifásicas ou trifásicas, as cargas devem ser distribuídas buscando o maior equilíbrio possível entre as fases.
\end{enumerate}
...
O memorial gerado segue uma estrutura padronizada, organizada em seções que refletem diretamente o pipeline descrito na Seção \todo{ADICIONAR REFERENCIA DA SEÇãO}, garantindo coerência entre: (i) informações de entrada, (ii) resultados intermediários e (iii) dimensionamento final.

\subsection{Organização do memorial}
O memorial é composto, de modo geral, pelas seguintes seções:

\begin{enumerate}
    \item \textbf{Dados da obra} \\
    ...
    \item \textbf{Objetivos e escopo do memorial} \\
    ...
    \item \textbf{Metodologia aplicada no memorial} \\
    ...
    \item \textbf{Levantamento de cargas (por ambiente)} \\
    ...
    \item \textbf{Divisão dos circuitos terminais} \\
    ...
    \item \textbf{Padrão de fornecimento e dimensionamento da entrada/proteção geral} \\
    ...
    \item \textbf{Dimensionamento dos dispositivos de proteção} \\
...
A estrutura do memorial é construída para manter correspondência direta com o pipeline descrito na Seção 3.3. Assim, cada tabela do memorial deriva de uma etapa do método (cargas → circuitos → condutores → proteções → fornecimento/balanceamento), permitindo auditoria e verificação dos resultados obtidos.
...
\subsection{Base de refrência e dados utilizados}
Como referência principal, foi adotado um projeto elétrico residencial disponibilizado em contexto didático no curso de instalações elétricas residenciais da Universidade Federal do Ceará (UFC), que inclui enunciado do projeto, solução de referência (gabarito do docente) e soluções típicas produzidas por discentes. Essa escolha se justifica por se tratar de material estruturado com finalidade pedagógica e alinhado às exigências normativas, permitindo uma comparação objetiva entre o resultado esperado e o resultado obtido pela metodologia.
